...
for i in range(40):
	if str(i+1) in sys.argv:
		...
		if (port in ports_power_5v):
		   error="Le port choisi est une alimentation 5v !"
		   finish=False
		if (port in ports_power_3v):
			error="Le port choisi est une alimentation 3.3v !"
			finish=False
		if (port in ports_ground):
			error="Le port choisis est la masse !(use -pinout)"
			finish=False
